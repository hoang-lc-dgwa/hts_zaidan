...
//    }
  ...
  $('#homestay_submitbtn').click(function(){
    ...
    if($('[name="新型コロナウイルスのワクチンは接種しましたか"]:checked').val() == "はい"){
      if($('[name="'+item_name+'"]:checked').length == 0){
        $('[name="'+item_name+'"]').closest('td').append('<div class="error error-up1">接種した回数は選択して下さい。</div>');
        error++;
      }
    }
    item_name = '接種した回数は[2]';
  ...
  if(errsize19 != null){
    ...
    if($('#agree_check2').prop('checked') != true){
      $('#agree_check2').closest("div.agree").append('<div class="error" style="background:none;">お申込み条件について同意頂く必要があります</div>');
      $("html,body").animate({scrollTop:$('.error').offset().top});
      //alert('お申込み条件について同意頂く必要があります');
      error2++;
    }
  ...
  $('#homestay2_submitbtn').click(function(){
