$(function(){
  ...
  $('#gakuseikaikan_submitbtn').click(function(){
    ...
    item_name = '宗教上の理由などで食べられないものがありますか';
    if($('[name="'+item_name+'"]:checked').length == 0){
      $('[name="'+item_name+'"]').closest("td").append('<div class="error">選択して下さい</div>');
      error++;
    }
    item_name = '食べられないもの';
    if($('[name="宗教上の理由などで食べられないものがありますか"]:checked').val() == "はい"){
      if($('[name="'+item_name+'"]').val() == ''){
        $('[name="'+item_name+'"]').closest('td').append('<div class="error">詳細を入力して下さい</div>');
        error++;
      }
    }
    ...
    if($('[name="問い合わせされた方[]"]:checked').length == 0){
      $('[name="問い合わせされた方[]"]').closest('td').append('<div class="error">問い合わせされた方選択して下さい</div>');
      error++;
    }
    if($('[name="当財団の学生会館を知ったきっかけ[]"]:checked').length == 0){
      $('[name="当財団の学生会館を知ったきっかけ[]"]').closest('td').append('<div class="error">当財団の学生会館を知ったきっかけ選択して下さい</div>');
      error++;
    }
    if($('[name="ご希望の寮[]"]:checked').length == 0){
      $('[name="ご希望の寮[]"]').closest('td').append('<div class="error">ご希望の寮選択して下さい</div>');
      error++;
    }
  ...
  $('#asia_submitbtn').click(function(){
    ...
    if($('[name="今回選んだ体験[]"]:checked').length == 0){
      $('[name="今回選んだ体験[]"]').closest('td').append('<div class="error">今回選んだ企画テーマ選択して下さい</div>');
      error++;
    }
    if($('[name="今回応募した企画を実施する国[]"]:checked').length == 0){
      $('[name="今回応募した企画を実施する国[]"]').closest('td').append('<div class="error">今回応募した企画を実施する国選択して下さい</div>');
      error++;
    }
...
//    }
    if($('[name="何をご覧になってご応募いただきましたか[]"]:checked').length == 0){
      $('[name="何をご覧になってご応募いただきましたか[]"]').closest('td').append('<div class="error">何をご覧になってご応募いただいたか選択して下さい</div>');
      error++;
    }
  ...
  $('#homestay_submitbtn').click(function(){
    ...
    item_name = '性別';
    if($('[name="'+item_name+'"]:checked').length == 0){
      $('[name="'+item_name+'"]').closest('td').append('<div class="error">'+item_name+'選択して下さい</div>');
      error++;
    }
    item_name = '配偶者';
    if($('[name="'+item_name+'"]:checked').length == 0){
    ...
    item_name = 'ホームステイ先';
    if($('[name="ホームステイをした経験"]:checked').val() == "はい"){
      if($('[name="'+item_name+'"]').val() == ''){
        $('[name="'+item_name+'"]').closest('td').append('<div class="error">詳細を入力して下さい</div>');
        error++;
      }
    ...
    item_name = '学んでいることは何ですか';
    if($('[name="'+item_name+'"]').val() == ''){
      $('[name="'+item_name+'"]').closest("td").append('<div class="error">学んでいることを入力して下さい</div>');
      error++;
    }
   ...
   if($('[name="'+item_name+'"]:checked').val() == "いいえ"){
    ...
    item_name = 'お酒を飲みますか'; // --6
    if($('[name="'+item_name+'"]:checked').length == 0){
      $('[name="'+item_name+'"]').closest('td').append('<div class="error">選択して下さい</div>');
      error++;
    }

    item_name = 'ゲストハウスに泊まったことはありますか'; // --7
    if($('[name="'+item_name+'"]:checked').length == 0){
      $('[name="'+item_name+'"]').closest('td').append('<div class="error">選択して下さい</div>');
      error++;
    }

    item_name = 'ゲストハウスは同じ部屋に複数人で泊まります問題ないですか'; // --8
    if($('[name="'+item_name+'"]:checked').length == 0){
      $('[name="'+item_name+'"]').closest('td').append('<div class="error">選択して下さい</div>');
      error++;
    }

    item_name = 'ゲストハウスは禁煙です問題ないですか'; // --9
    if($('[name="'+item_name+'"]:checked').length == 0){
      $('[name="'+item_name+'"]').closest('td').append('<div class="error">選択して下さい</div>');
      error++;
    }

    item_name = '動物と2日間同居します問題ないですか'; // --14
    if($('[name="'+item_name+'"]:checked').length == 0){
      $('[name="'+item_name+'"]').closest('td').append('<div class="error">選択して下さい</div>');
      error++;
    }

    item_name = '宿泊先のオーナー夫妻にメッセージを書いてください'; // --15
    if($('[name="'+item_name+'"]').val() == ''){
      $('[name="'+item_name+'"]').closest("td").append('<div class="error">宿泊先のオーナー夫妻にメッセージを書いてください</div>');
      error++;
    }

    item_name = 'その他、聞きたいことや特記事項があれば記入して下さい'; // --16
    if($('[name="'+item_name+'"]').val() == ''){
      $('[name="'+item_name+'"]').closest("td").append('<div class="error">その他、聞きたいことや特記事項があれば記入して下さい</div>');
      error++;
    }
  ...
  if(errsize18 != null){
    if(errsize18.value == '') {
       if( $('[name="'+item_name+'"]:checked').length == 0){
          $('[name="'+item_name+'"]').closest('td').append('<div class="error">アウターのサイズは何ですか選択して下さい</div>');
          error++;
        }
    }
  }
  ...
  if(errsize19 != null){
    if(errsize19.value == '') {
       if( $('[name="'+item_name+'"]:checked').length == 0){
          $('[name="'+item_name+'"]').closest('td').append('<div class="error">ズボンのサイズは何ですか選択して下さい</div>');
          error++;
        }
    }
  }
    ...
    item_name = 'いいえの方のみ 何歳以上なら大丈夫ですか';
    ...
    if($('[name="'+item_name+'"]').val() == ''){
      $('[name="'+item_name+'"]').closest('td').append('<div class="error">ホストファミリーにメッセージを書いてください</div>');
      error++;
    }
    ...
    item_name = 'いいえの方のみそれは何ですか';
    if($('[name="ホームステイ先にペット（犬・猫など）がいても大丈夫ですか"]:checked').val() == "いいえ"){
      if($('[name="'+item_name+'"]').val() == ''){
        $('[name="'+item_name+'"]').closest('td').append('<div class="error">詳細を入力して下さい</div>');
        error++;
      }
    ...
    if($('[name="何かアレルギーを持っていますか"]:checked').val() == "はい"){
      if(($('[name="どんなアレルギーですか[1]"]').val() == '') && ($('[name="どんなアレルギーですか[2]"]').val() == '') && ($('[name="どんなアレルギーですか[3]"]').val() == '')){
        $('[name="どんなアレルギーですか[3]"]').closest('td').append('<div class="error error-up1">詳細を入力して下さい</div>');
        error++;
      }
    }
    ...
    if($('#upfile01').val() == ''){
      $('#upfile01').closest("td").append('<div class="error">表面を添付して下さい</div>');
      error++;
    }
    if($('#upfile02').val() == ''){
      $('#upfile02').closest("td").append('<div class="error">表面を添付して下さい</div>');
      error++;
    }
    if($('#upfile03').val() == ''){
      $('#upfile03').closest("td").append('<div class="error">顔写真を添付して下さい</div>');
  ...
  $('#homestay_kushiro_submitbtn').click(function(){
    ...
    if($('#upfile02').val() == ''){
      $('#upfile02').closest("td").append('<div class="error">裏面を添付して下さい</div>');
      error++;
    }
    if($('#upfile03').val() == ''){
      $('#upfile03').closest("td").append('<div class="error">顔写真を添付して下さい</div>');
